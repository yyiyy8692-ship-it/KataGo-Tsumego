...
C_MARK = "#D85A30"        # 最新一手高亮环（珊瑚色，与黑白子都区分得开）
...
def board_svg(name, black, white, corner, cols=7, rows=9,
    ...
    mark: 最新一手 (x, y)，画珊瑚色高亮环；seq: 该手的序号，写在棋子中心
    （变化图用——看清走了第几手、落在哪）。sub: 覆盖底部副行文字。

    返回 SVG 字符串。corner ∈ TL/TR/BL/BR，决定哪两条边画成石墙。
    """
    w, h, inner = board_parts(name, black, white, corner, cols, rows,
                              cell, margin, mark, seq, sub)
    return (f'<svg viewBox="0 0 {w} {h}" xmlns="http://www.w3.org/2000/svg" '
            f'style="max-width:100%;background:{C_BG}">{inner}</svg>')
...
def board_parts(name, black, white, corner, cols=7, rows=9,
    ...
    if sub is None:
        sub = f"{CORNER_LABEL[corner]} ｜ 黑 {len(black)} · 白 {len(white)}"
    p.append(f'<text x="{w / 2}" y="{ty + 26}" font-size="14" '
             f'fill="{C_SUB}" text-anchor="middle" '
             f'font-family="PingFang SC, sans-serif">{html.escape(sub)}</text>')
    return w, h, "".join(p)
